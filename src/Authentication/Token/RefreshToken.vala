...
namespace XboxWebApi.Authentication.Token {
    public class RefreshToken : BaseAuthToken {
        public RefreshToken.from_windows_live_response (WindowsLiveResponse response) {
            issued = response.creation_time;
            expires = response.creation_time.add_days (14);
            jwt = response.refresh_token;
        }
    }
}
